 ...
 * such as those found on 8-bit retrocomputers.  A monochromatic image, with
 * the character patterns in a regular grid, must be supplied.  Chromatic
...
yoob.Chargen = function() {
    this.init = function(colourTriples, charWidth, charHeight,
                         charsPerRow, rows, imageSrc, onLoad) {
        this.colourTriples = colourTriples;
        this.bitmaps = [];  // canvas elements

        for (var i = 0; i < colourTriples.length; i++) {
     ...
     */
    this.blitChar = function(charNum, charColour, ctx, x, y,
        ...
        ctx.drawImage(this.bitmaps[charColour],
    ...
    this.reset = function() {
        ...
        this.createChromaticBitmaps();
    };

    /*
     * Probably you will never need to call this directly.
     */
    this.createChromaticBitmaps = function() {
        var ctx = this.bitmaps[0].getContext('2d');
        var w = this.img.width;
        var h = this.img.height;
        var imageData = ctx.getImageData(0, 0, w, h);
        for (var color = 1; color < this.colourTriples.length; color++) {
            ...
            for (var y = 0; y < h; y++) {
                for (var x = 0; x < w; x++) {
                    ...
                    newData.data[index] = this.colourTriples[color][0];
                    newData.data[index + 1] = this.colourTriples[color][1];
                    newData.data[index + 2] = this.colourTriples[color][2];
